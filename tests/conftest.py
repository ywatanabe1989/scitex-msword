...
Also wires subprocess coverage so any child Python interpreter
(`subprocess.run([sys.executable, ...])`, `jupyter nbconvert --execute`,
etc.) writes coverage data alongside the parent. See
``scitex_dev/_skills/general/05_development_06_subprocess-coverage.md``.

`os.environ.setdefault` would be a silent no-op here because pytest-cov
has already set ``COVERAGE_FILE`` to a tmp dir by the time conftest is
loaded; we therefore force-set both env vars at module import time.
"""
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_msword_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
